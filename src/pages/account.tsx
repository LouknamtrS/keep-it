...
export default function Account(){
    ...
    useEffect(() => {
        setIsChanged(username !== initialData.username);
    }, [username]);

    const handleSave = () => {
        try {
            showSuccess("แก้ไขข้อมูลสำเร็จ", "ข้อมูลของคุณได้รับการอัปเดตแล้ว");
        } catch (error) {
            ...
            <div className="flex w-screen h-screen items-start justify-center mt-12">
                <div className="flex flex-col gap-6 pt-10 px-6 sm:px-12 pb-10 bg-white rounded-2xl shadow-none sm:shadow-sm sm:border sm:border-gray-100 border-none w-full max-w-xl mx-auto mt-10">
                    <div className="flex flex-row items-center w-full relative">
                        ...
                        <h1 className="text-xl  text-gray-800 text-center w-full">การตั้งค่าบัญชีผู้ใช้</h1>
                    </div>
                    <div className="flex flex-col gap-4 sm:mx-20">
                        <div className="flex flex-col items-center gap-4">
                            <div className="relative h-24 w-24">
                                <img src={mock_profile} alt="Profile" className="h-full w-full object-cover rounded-full"/>
                                <button className="absolute bottom-0 right-0 bg-primary-10 text-primary-50 p-2 rounded-full hover:bg-primary-60 hover:text-white transition-colors cursor-pointer flex items-center justify-center">
                                    <i className="bi bi-pencil-fill text-xs"></i>
                                </button>
                            </div>
                        </div>
                        <div className="flex flex-col gap-2">
                            <label className="text-sm text-primary-70 text-left pl-4">ชื่อผู้ใช้</label>
                        ...
                        <div className="flex flex-col gap-2">
                            <label className="text-sm text-primary-70 text-left pl-4">อีเมล</label>
                            <div className="relative">
                                ...
                                <input 
                                    ...
                                    disabled
                                    className="bg-gray-10 border border-gray-20 rounded-2xl py-2 pl-12 pr-4 w-full focus:outline-none focus:ring-1 focus:ring-primary-40 text-gray-40"
                                />
                            </div>
                        </div>
                        ...
                        <button
                            ...
                            className={`mt-6 w-full py-3 rounded-2xl transition-all duration-200 ${
                                isChanged 
                                ? "bg-primary-70 text-white hover:bg-primary-90 cursor-pointer  active:scale-[0.98]" 
